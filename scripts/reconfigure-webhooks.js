...
async function listWebhooks() {
    ...
    try {
        const response = await makeRequest(`https://api.figma.com/v2/webhooks?team_id=${TEAM_ID}&context=team&context_id=${TEAM_ID}`, options);
...
// Fonction pour créer un webhook avec LIBRARY_PUBLISH
async function createWebhook(fileId, fileName) {
    console.log(`🚀 Création du webhook LIBRARY_PUBLISH pour ${fileName} (${fileId})...`);
    ...
    try {
        ...
        if (response.status === 200 || response.status === 201) {
            console.log(`✅ Webhook LIBRARY_PUBLISH créé pour ${fileName}`);
...
// Fonction principale
async function main() {
    console.log('🔄 Reconfiguration des webhooks avec LIBRARY_PUBLISH');
    console.log('==================================================');
    ...
    if (existingWebhooks.length > 0) {
        console.log('\n🗑️  Suppression des webhooks FILE_UPDATE existants...');
        let deletedCount = 0;
        
        for (const webhook of existingWebhooks) {
            console.log(`   - ${webhook.id}: ${webhook.event_type} → ${webhook.endpoint}`);
    ...
    // 2. Créer les 3 webhooks avec LIBRARY_PUBLISH
    console.log('\n🚀 Création des webhooks LIBRARY_PUBLISH pour les 3 fichiers principaux...');
    ...
    console.log(`✅ ${createdCount}/3 webhooks LIBRARY_PUBLISH créés`);
    console.log('\n💡 Maintenant teste en publiant dans Core Primitives !');
}
